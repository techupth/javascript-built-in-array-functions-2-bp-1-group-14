const bills = [
  ...
  const result = Object.values(bills.reduce((value,object) =>{
    if(value[object.location]){
        value[object.location].total += object.total; 
    }else{
        value[object.location] = {...object, 
    };
    }
    return value;
  }, {}));
...
const callbackFn2 = (element) => {
    return [element.location , element.total]
   }
   
   let text = result.map(callbackFn2)
   console.log(text)
